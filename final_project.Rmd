...
agg_day = agg %>% group_by(`Start Terminal`, day_of_week, lat, long) %>% summarise(total = sum(`Number of Trips`)) %>% group_by(`Start Terminal`, lat, long) %>% summarise(avg = mean(total))
