...
agg = merge(df, station, by.x = 'End Terminal', by.y = 'station_id') %>% mutate(day_of_week = wday(date))
...
r = raster(nrows=10, ncol=20, xmn = min(bay_county$long), xmx = max(bay_county$long),ymn = min(bay_county$lat), ymx = max(bay_county$lat))
...
trip_pred = r
trip_pred[cells] = predict(tps, sf_pred_coords)
